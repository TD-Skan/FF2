...
async function loadData(auto=false){
  try{
    ...
    $("predictButton").disabled=true; $("downloadButton").disabled=true;
    bootstrapData=await fetchJson(BOOTSTRAP_URL); allFixtures=await fetchJson(FIXTURES_URL); metadata=await fetchJson(METADATA_URL,true);
    validateData(); currentGameweekId=detectGameweek().id; fixturesData=allFixtures.filter(f=>Number(f.event)===Number(currentGameweekId)); allPlayers=buildAllPlayers(false); populateControls();
    $("summarySeason").textContent=metadata?.inferred_season||inferSeason(bootstrapData.events); $("summaryGameweek").textContent=`GW${currentGameweekId}`; $("predictButton").disabled=false;
...
function populateControls(){
  $("gameweekSelect").innerHTML='<option value="auto">Auto detect</option>'; bootstrapData.events.forEach(e=>{ const o=document.createElement('option'); o.value=e.id; o.textContent=`${e.name||"GW"+e.id} (${e.id})`; $("gameweekSelect").appendChild(o); });
  const opts=allPlayers.slice().sort((a,b)=>a.name.localeCompare(b.name)); $("playerOptions").innerHTML=opts.map(p=>`<option value="${esc(p.name)}">${esc(p.team)} · ${esc(p.position)} · ${money(p.price)}</option>`).join('');
  const html='<option value="">Select player</option>'+opts.map(p=>`<option value="${p.id}">${esc(p.name)} - ${esc(p.team)} (${esc(p.position)})</option>`).join(''); $("compareA").innerHTML=html; $("compareB").innerHTML=html; $("playerSearchButton").disabled=false; $("compareButton").disabled=false;
}
function buildFixtureLookup(){ const raw=new Map(); fixturesData.forEach(f=>{ if(!raw.has(Number(f.team_h)))raw.set(Number(f.team_h),[]); if(!raw.has(Number(f.team_a)))raw.set(Number(f.team_a),[]); raw.get(Number(f.team_h)).push(n(f.team_h_difficulty)); raw.get(Number(f.team_a)).push(n(f.team_a_difficulty)); }); const out=new Map(); raw.forEach((vals,teamId)=>out.set(teamId,{averageDifficulty:r(vals.reduce((a,b)=>a+b,0)/vals.length,2),fixtureCount:vals.length})); return out; }
function scorePlayer(p,fxLookup){
  const cfg=profile(), epNext=n(p.ep_next), epThis=n(p.ep_this), form=n(p.form), ppg=n(p.points_per_game); let base=ppg; if(epNext>0)base=epNext; else if(epThis>0)base=epThis; else if(form>0)base=form;
  let fixtureMult=.70; const fx=fxLookup.get(Number(p.team)); if(fx){ fixtureMult=1+((3-fx.averageDifficulty)*.08); if(fx.fixtureCount>1)fixtureMult+=(fx.fixtureCount-1)*.15; }
  const mins=n(p.minutes); let minutesMult=mins>=900?1:mins>=450?.85:mins>0?.65:.35; minutesMult=1+((minutesMult-1)*cfg.safe);
  let availMult=.4; if(p.chance_of_playing_next_round!==null&&p.chance_of_playing_next_round!==undefined)availMult=n(p.chance_of_playing_next_round)/100; else if(p.status==='a')availMult=1; availMult=1+((availMult-1)*cfg.safe);
  const own=n(p.selected_by_percent); let pop=own>=20?1.04:own>=10?1.02:1; if(cfg.differential>1 && own<10) pop*=cfg.differential;
  const pos={1:'GKP',2:'DEF',3:'MID',4:'FWD'}[Number(p.element_type)]||'UNK'; const posBoost=cfg.positionBoost[pos]||1; const upsideBoost=1+((fixtureMult-1)*(cfg.upside-1));
  return r(base*fixtureMult*minutesMult*availMult*pop*posBoost*upsideBoost,2);
}
function buildAllPlayers(applyChance=true){ const teams=lookup(bootstrapData.teams,'id','name'), positions=lookup(bootstrapData.element_types,'id','singular_name_short'), fx=buildFixtureLookup(), minChance=Number($("chanceInput")?.value??0); return bootstrapData.elements.filter(p=>{ if(!applyChance)return true; if(p.status==='a')return true; const c=p.chance_of_playing_next_round; if(c===null||c===undefined)return minChance===0; return Number(c)>=minChance; }).map(p=>{ const price=r(n(p.now_cost)/10,1), pred=scorePlayer(p,fx); return {id:Number(p.id),name:p.web_name||`${p.first_name} ${p.second_name}`,fullName:`${p.first_name||''} ${p.second_name||''}`.trim(),teamId:Number(p.team),team:teams.get(Number(p.team))||'Unknown',position:positions.get(Number(p.element_type))||'UNK',nowCost:Number(p.now_cost),price,totalPoints:Number(p.total_points)||0,form:n(p.form),pointsPerGame:n(p.points_per_game),expectedNext:n(p.ep_next),expectedThis:n(p.ep_this),minutes:Number(p.minutes)||0,selectedByPercent:n(p.selected_by_percent),status:p.status,news:p.news||'',goals:Number(p.goals_scored)||0,assists:Number(p.assists)||0,cleanSheets:Number(p.clean_sheets)||0,bonus:Number(p.bonus)||0,predictedPoints:pred,valueScore:price>0?r(pred/price,2):0}; }); }
...
async function predictTeam(){ try{ if(!bootstrapData)throw new Error('Data is still loading.'); $("predictButton").disabled=true; $("downloadButton").disabled=true; currentGameweekId=selectedGameweek(); fixturesData=allFixtures.filter(f=>Number(f.event)===Number(currentGameweekId)); allPlayers=buildAllPlayers(false); buildPlayerPool(); const remaining=selectSquad(); const xi=bestXI(); if(!xi.players.length)throw new Error('Selected formation could not be built from the generated squad. Choose Auto best formation.'); startingXI=xi.players; const ids=new Set(startingXI.map(p=>p.id)); bench=selectedSquad.filter(p=>!ids.has(p.id)).sort(sortPred); render(xi,remaining); $("downloadButton").disabled=false; status(`Prediction complete for GW${currentGameweekId}.`); }catch(e){ console.error(e); status(e.message,true); } finally{ $("predictButton").disabled=false; } }
